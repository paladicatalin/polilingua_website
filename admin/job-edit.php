...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$title_ro) {
        ...
        $locationRo = sanitize($_POST['location'] ?? '');
        $scheduleRo = sanitize($_POST['schedule'] ?? '');
        ...
        $stmt = $db->prepare("
            UPDATE jobs SET
                ...
                location=?, schedule=?, sticky_color=?, sticky_rotation=?,
        ...
        header('Location: jobs.php?msg=updated&translation=' . rawurlencode($translationStatus));
        exit;
    }
}

// Pre-fill from POST or DB
$f = $_SERVER['REQUEST_METHOD'] === 'POST' ? $_POST : $job;

$adminTitle = 'Editează post: ' . e($job['title_ro']);
  ...
  <div class="card-body">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        ...
        <input type="text" name="title_ro" required value="<?= e($f['title_ro'] ?? '') ?>">
      </div>
      ...
      <div class="form-group">
        ...
        <textarea name="short_desc_ro" rows="3"><?= e($f['short_desc_ro'] ?? '') ?></textarea>
      </div>

      <h3 style="margin:24px 0 16px;font-size:0.95rem;color:#64748b;border-bottom:1px solid #e2e8f0;padding-bottom:10px;">Descriere completă</h3>
      <div class="form-group">
        <label>Descriere completă Română</label>
        <textarea name="full_desc_ro" rows="6"><?= e($f['full_desc_ro'] ?? '') ?></textarea>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>Locație</label>
          <input type="text" name="location" value="<?= e($f['location'] ?? '') ?>">
        </div>
        <div class="form-group">
          <label>Program</label>
          <input type="text" name="schedule" value="<?= e($f['schedule'] ?? '') ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Culoare sticky note</label>
          <input type="color" name="sticky_color" value="<?= e($f['sticky_color'] ?? '#4CAF82') ?>" style="height:42px;padding:4px;">
        </div>
        <div class="form-group">
          <label>Rotație (grade)</label>
          <input type="number" name="sticky_rotation" step="0.5" min="-15" max="15" value="<?= e($f['sticky_rotation'] ?? '-2.5') ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Ordine afișare</label>
          <input type="number" name="sort_order" min="0" value="<?= e($f['sort_order'] ?? '0') ?>">
        </div>
        <div class="form-group" style="display:flex;align-items:center;gap:12px;margin-top:28px;">
          <input type="checkbox" name="is_active" id="is_active" value="1" <?= ($f['is_active'] ?? 1) ? 'checked' : '' ?> style="width:18px;height:18px;">
